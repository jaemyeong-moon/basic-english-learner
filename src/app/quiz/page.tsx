...
import { QuizQuestion as QuizQuestionType, QuizResult } from "@/types";
import { getQuizQuestions, getVerbById } from "@/lib/utils";
...
export default function QuizPage() {
  ...
  const restart = () => {
    ...
    setQuestions(getQuizQuestions(QUIZ_COUNT));
    setCurrentIndex(0);
  ...
  // 최근 3회 히스토리 (현재 결과 제외 — 저장 후 반영되므로 slice로 표시)
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-8 text-center dark:border-zinc-700 dark:bg-zinc-900">
          ...
          <div className="mb-8 space-y-3 text-left">
            {result.answers.map((ans, i) => {
              ...
              const verb = getVerbById(q.verbId);
              return (
                <div
                  key={i}
